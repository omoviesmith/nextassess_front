...
//                     },
//                     body: JSON.stringify(formValues)   // Send JSON string instead of FormData
//                 });
...
//                             <p className="text-center text-base font-semibold mt-3">Sign in to your account</p>
        ...
        try {
            ...
            if (Object.keys(newErrors).length === 0) {
                ...
                if (res.ok) {
                    ...
                    toast.success('Logged in successfully!');
                    setUser(parsedResponse.user_details); // Uses UserContext to set user
                    console.log(typeof(parsedResponse.user_details))
                    router.push('/assessments');
                } else {
                    setLoading(false);
                    toast.error('Something went wrong. Please try again!');
